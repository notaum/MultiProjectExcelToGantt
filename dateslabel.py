...
if len(sys.argv) < 2:
    print("Usage: python dateslabel.py name.csv (optional) --dark (optional) Height Multiplier")
    sys.exit(1)
...
try:
    ...
    fig = px.timeline(
        ...
        text='Task'  # Add task labels inside bars
    )
    ...
    # Create date labels: day numbers every 2 days, with month on 1st and 15th
    current = extended_start
    tickvals = []
    ticktext = []
    
    while current <= extended_end:
        # Add tick for every 2 days
        tickvals.append(current)
        
        # Format label: show month on 1st and 15th, otherwise just day number
        if current.day == 1 or current.day == 15:
            month_abbr = current.strftime('%b')  # 3-letter month abbreviation
            ticktext.append(f"{current.day} {month_abbr}")
        else:
            ticktext.append(str(current.day))
    ...
    # Style traces - make bars fully solid with slight rounding and add text labels
    fig.update_traces(
        marker=dict(
            line_width=1,
            opacity=1.0,
    ...
    # Export to HTML
    output_file = f'output{output_suffix}.html'
